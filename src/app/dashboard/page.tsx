...
// --- 1. THERMAL PRINTER OPTIMIZED RECEIPT (80mm / 3 inch) ---
const PrintReceipt = ({ order, cart, settings }: any) => {
  ...
  return (
    <div className="w-[76mm] mx-auto p-2 bg-white text-black font-mono leading-tight overflow-visible">
      ...
      {/* KITCHEN TOKEN (PART OF SAME BILL) */}
...
export default function Dashboard() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // Delay to ensure mobile browsers and QR codes render before the print dialog
      setTimeout(() => { 
        window.print(); 
        setPrintData(null); 
        setCart([]); 
  ...
  return (
    <div className="flex h-screen bg-[#020617] overflow-hidden text-slate-100">
      ...
      {/* DESKTOP SIDEBAR */}
      <aside className="no-print hidden lg:flex w-20 bg-slate-900 flex-col items-center py-8 gap-10 border-r border-slate-800">
      ...
      {/* DASHBOARD HUB */}
      <main className="no-print flex-1 flex flex-col min-w-0 bg-slate-950">
        <header className="px-6 py-4 flex items-center justify-between border-b border-slate-900">
          <h1 className="text-xl font-black uppercase tracking-widest text-red-700">Transaction Hub</h1>
          <div className="relative max-w-xs w-full hidden md:block">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-600" size={16} />
            <input type="text" placeholder="Search menu..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-slate-900 border-none pl-10 pr-4 py-2 rounded-lg text-sm focus:ring-1 focus:ring-red-900" />
          </div>
        ...
        <div className="p-4 flex gap-2 overflow-x-auto no-scrollbar">
          {categories.map(cat => (
            <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-wider transition-all whitespace-nowrap ${activeCategory === cat ? 'bg-red-800 text-white' : 'bg-slate-900 text-slate-500 hover:bg-slate-800'}`}>{cat}</button>
          ))}
        </div>

        <div className="flex-1 overflow-y-auto p-4 grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-6 gap-4 no-scrollbar">
          {filteredProducts.map(p => (
            <div key={p.id} onClick={() => addToCart(p)} className="bg-slate-900/50 p-4 rounded-2xl border border-slate-900 hover:border-red-900 transition-all cursor-pointer group">
              <div className="aspect-square bg-slate-900 rounded-xl mb-3 flex items-center justify-center text-3xl group-hover:scale-105 transition-transform">{p.category === 'Drinks' ? '🥤' : '🍔'}</div>
              <h3 className="font-bold text-xs uppercase truncate">{p.name}</h3>
              <p className="text-red-600 font-black text-sm mt-1 italic">₹{p.price_exclusive_tax}</p>
            </div>
          ))}
        </div>
      </main>
      ...
      {/* CART (SIDEBAR SLIDER) */}
      <aside className={`no-print fixed lg:relative inset-y-0 right-0 z-[120] w-full lg:w-[380px] bg-slate-900 flex flex-col transition-transform duration-300 shadow-2xl ${isCartOpen ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'}`}>
        <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-900">
          <h2 className="font-black uppercase tracking-tighter text-lg">Order Details</h2>
        ...
        <div className="px-6 py-4">
          ...
          <div className="relative">
            <button onClick={() => setIsTableDropdownOpen(!isTableDropdownOpen)} className="w-full bg-slate-950 p-3 rounded-xl flex justify-between items-center text-sm font-bold border border-slate-800">
            ...
            {isTableDropdownOpen && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-slate-800 p-2 rounded-xl grid grid-cols-5 gap-1 shadow-2xl z-50 border border-slate-700">
        ...
        <div className="p-6 bg-slate-950 border-t border-slate-800 space-y-4">
          ...
          <button onClick={handleCheckout} disabled={cart.length === 0 || loading} className="w-full bg-red-800 hover:bg-red-700 disabled:opacity-50 text-white font-black py-5 rounded-xl flex items-center justify-center gap-2 uppercase tracking-widest text-[11px] shadow-lg">
            {loading ? <Loader2 className="animate-spin" /> : <><Printer size={18} /> Complete Order & Print</>}
          </button>
        </div>
      </aside>
      ...
      {/* --- PRINT AREA (ISOLATED) --- */}
      {printData && (
        <div id="receipt-print-area" className="hidden print:block fixed inset-0 z-[999] bg-white">
      ...
      <style jsx global>{`
        @media print {
          ...
          #receipt-print-area { 
            ...
            left: 0; 
            top: 0; 
            width: 76mm; /* Perfect for 3-inch printers */
...
function NavItem({ icon, active, onClick }: { icon: any, active?: boolean, onClick?: () => void }) {
  return (
    <div onClick={onClick} className={`p-3 rounded-xl cursor-pointer transition-all ${active ? 'bg-red-800 text-white shadow-xl' : 'text-slate-500 hover:bg-slate-800 hover:text-white'}`}>
      {icon}
    </div>
  );
}